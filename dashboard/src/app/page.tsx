...
function readingsToState(readings: SensorReading[]): RoomState {
  ...
  for (const r of readings) {
    ...
    if (!state.lastUpdate || r.created_at > state.lastUpdate) {
      state.lastUpdate = r.created_at;
    }
  }
  return state;
}
...
export default function DashboardPage() {
  ...
  const loadInitial = useCallback(async () => {
    try {
      const [r1, r2] = await Promise.all([
        fetchLatestReadings(1),
        fetchLatestReadings(2),
      ]);
      setRoom1(readingsToState(r1));
      setRoom2(readingsToState(r2));
    } catch (err) {
      console.error('[Dashboard] Error cargando datos iniciales:', err);
    } finally {
  ...
  useEffect(() => {
    ...
    // Realtime: escuchar nuevas inserciones del bridge
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Datos actualizados automáticamente desde el ESP32 vía HiveMQ
        </p>
      </div>
